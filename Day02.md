...
ex) 이름 : 홍길동(num1), 별명 : 도적(ref) -> 홍길동과 홍길동의 별명은 같은 대상을 가르키기에 뭐라 부르던 같은 한 사람!
    ...
    /* C++에서는 매개변수를 참조(&)로 받을 수 있다. 또한 함수가 참조자를 반환(return)하려면 반환형도 참조(&)로 선언해야 한다. */
...
#### 정리
1. 컴파일(읽는 순서) : 위 → 아래
2. 실행(동작 순서) : main() → 호출된 함수 → main()으로 복귀
3. C++에서는 매개변수를 참조(&)로 받을 수 있으며 함수가 참조자를 반환(return)하려면 반환형도 참조(&)로 선언해야 한다.
4. 변수 값을 모든 함수, main에서 사용하기 위해 참조자 사용 (c, c++ 은 변수만으로는 불가능)
5. int& 도 어쨌든 int 형태이므로 int로 받을 수 있음 ( 전달 : int& -> 받음 : int )
6. 함수 안에 있는 변수에는 반드시 별명, 주소 형태로 선언하여야 main의 변수 값을 가져오던가 함수에서 return 하여 전달할 수 있음
