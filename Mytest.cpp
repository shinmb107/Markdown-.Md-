...
public:
    ...
    // 디폴트 매개변수 -> A() : 를 사용하면 A(), A(int), A(int,int)을 하나의 생성자로 처리 가능

	A (int n = 0, string str = "Hello") : num(0), ch("Hello") : num(n), ch(str) {	// 구조 -> 생성자 이름() : 이니셜라이저로 멤버 초기화
    ...
    // 소멸자
    // 객체가 소멸될 때 자동으로 호출된다.
    // 동적 메모리를 사용한 경우 메모리 해제 등의 마무리 작업을 수행한다.
    // 소멸자는 매개변수와 반환형을 가질 수 없으며 클래스당 하나만 존재한다.
